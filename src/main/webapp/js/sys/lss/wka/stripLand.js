...
function fncStripLandDetail(id) {
	...
	$("#listForm").attr("action","/sys/lss/bsc/sld/selectStripLandDetail.do");
	$("#listForm").submit();
}
...
function fncUploadStripLandInsert(){
	...
	var jqXHR=$.ajax({
		url:"/sys/lss/bsc/sld/uploadStripLandExcel.do",
		type:"POST",
		contentType:false,
        processData: false,
        cache: false,
...
function fncAddStripLandInsertView() {
	location.href="/sys/lss/bsc/sld/registStripLandView.do";
}
...
function fncDeleteStripLand(id){
	if(confirm("삭제하시겠습니까?")){
		...
		$("#listForm").ajaxForm({
			type: 'POST',
        	url: "/sys/lss/bsc/sld/removeLssBscStripLand.do",
        	dataType: "json",
        	success: function(data) {
        		if (data.status == "success") {
                	alert(data.message);
...
function fncUpdateStripLandView(){
	$("#listForm").attr("action","/sys/lss/bsc/sld/updateLssBscStripLandView.do");
	$("#listForm").submit();
}
