...
function fnInsertBscSvyPopultnPopup(){
	$('<div id="uploadDiv">').dialog({
		modal:true,
		open: function(){
			$(this).load("insertBscSvyPopultnPopup.do");
		},
		close: function(e){
			$(this).empty();
			$(this).dialog("destroy");
...
function fnInsertBscSvyPopultn(){
	...
	if(confirm("기초조사 모집단 건수를 변경하시겠습니까?")) {		
		$.ajax({
			url:"/sys/lss/bsc/sts/insertBscSvyPopultn.do",
			type:"POST",
        	data: {
        		mberId: mberId,
