...
function fnInsertFieldBookView(){
	...
	var form = $("<form></form>");
	$(form).attr("id","listForm").attr("name","itemVO").attr("method","post").attr("action","/ext/lss/cnl/upload.do").attr("enctype","multipart/form-data");
...
function fnSubSearch(){	
	...
	$("#searchForm").attr("action","/sys/lss/cnl/fbk/selectFieldBookDetail.do");
	        ...
	        success: function(data){
	        	if(data.nnt == 'fail') {
					...
					if(data.cnt == 'fail') {
		        		...
		        		$(".loading-div").remove();
		            	$(".progress").remove();
		            	$("#insertForm input[name=mst_partname]").val('');
						return false;
		        	}else{
						if(data.message){
			        		alert(data.message);
...
function fnFieldBookDetail(id) {
	...
	$("#listForm").attr("action","/sys/lss/cnl/fbk/selectFieldBookDetail.do");
...
function fnUpdateCnrsSpceView(id) {
	...
	$("#listForm").attr("action","/sys/lss/cnl/fbk/updateFieldBookView.do");
	$("#listForm").submit();
}
...
function fnUpdateCnrsSpce(label){
	...
	if(confirm("선택항목을 삭제하시겠습니까?")) {		
		$.ajax({
			url:"/sys/lss/cnl/fbk/updateCnrsSpceItem.do",
			type:"POST",
        	data: {
        		label: label,
	        	mst_id: mst_id
...
function fnDeleteCnrsSpce(id){
	if(confirm("공유방을 삭제하시겠습니까?")) {
		if(confirm("공유방삭제시 조사된 정보도 함께 삭제가 됩니다.\n그래도 삭제하시겠습니까?")){
			$.ajax({
				url:"/sys/lss/cnl/fbk/deleteCnrsSpce.do",
				type:"POST",
		        data: {
		        	id: id
		        },
		        ...
		        success:function(data){
		        	if(data.status == "success"){
		        		alert(data.message);
		        		location.href = "/sys/lss/cnl/fbk/selectFieldBookList.do"
		        	} else {
		        		alert(data.message);
		        	}
		        },
...
function fnSelectFieldBookDetailOne(smid){
	...
	$("#listForm").attr("action","/sys/lss/cnl/fbk/selectFieldBookDetailOne.do").attr("method", "post");
	$("#listForm").submit();
}
...
function fnSelectFieldBookUpdtOne(smid){
	...
	$("#listForm").attr("action","/sys/lss/cnl/fbk/updateFieldBookViewOne.do").attr("method", "post");
	$("#listForm").submit();
}
...
function fnUpdateFieldBookDetailOne(){
	...
	var svyLatD = parseInt($('input[name=svy_latD]').val());
	var svyLatM = parseInt($('input[name=svy_latM]').val());
	var svyLatS = parseFloat($('input[name=svy_latS]').val());
	
	var svyLonD = parseInt($('input[name=svy_lonD]').val());
	var svyLonM = parseInt($('input[name=svy_lonM]').val());
	var svyLonS = parseFloat($('input[name=svy_lonS]').val());
	...
	if(confirm("수정하시겠습니까?")) {
		$.ajax({
			url: "/sys/lss/cnl/fbk/updateFieldBookDetailOne.do",
	        ...
	        success:function(data){
	        	if(data.status == "success"){
	        		alert(data.message);
	        		fnList("/sys/lss/cnl/fbk/selectFieldBookList.do");
	        	} else {
	        		alert(data.message);
	        	}
	        },
...
function fnDeleteFieldBookDetailOne(){
	...
	if(confirm("삭제하시겠습니까?")) {
		$.ajax({
			url: "/sys/lss/cnl/fbk/deleteFieldBookDetailOne.do",
			data:$("#listForm").serialize(),
			dataType: "json",
			type:"POST",
			async:true,
			success:function(data){
				if(data.status == "success"){
					alert(data.message);
					window.location.href = "/sys/lss/cnl/fbk/selectFieldBookDetail.do?mstId="+mstId;
				} else {
					alert(data.message);
				}
			},
...
//		url:"/sys/lss/lcp/fbk/selectLcpSldListAddr.do",
		url:"/sys/lss/cnl/fbk/selectMberList.do",
		type:"POST",
        data: {
...
function fnManageCnrsSpceAuthory(id) {
	...
	$("#listForm").attr("action","/sys/lss/cnl/fbk/selectFieldbookAuthoryView.do");
	$("#listForm").submit();
}
