...
function fncStripLandDetail(id) {
	...
	$("#listForm").attr("action","/sys/lss/cnl/sld/selectStripLandDetail.do");
	$("#listForm").submit();
}
...
function fncUploadStripLandInsert(){
	...
	var jqXHR=$.ajax({
		url:"/sys/lss/cnl/sld/uploadStripLandExcel.do",
		type:"POST",
		contentType:false,
        processData: false,
        cache: false,
...
function fncAddStripLandInsertView() {
	location.href="/sys/lss/cnl/sld/registStripLandView.do";
}
        ...
        if(!validateLssCnlStripLand(form)){
        	...
        	$("#listForm").ajaxForm({
	        	...
	        	success: function(data) {
	        		if (data.status == "success") {
                    	alert(data.message);
                    	location.href="/sys/lss/cnl/sld/selectStripLandList.do";
                	} else {
	                    alert(data.message);
	                }
    	    	},
...
function fncDeleteStripLand(id){
	if(confirm("삭제하시겠습니까?")){
		...
		$("#listForm").ajaxForm({
			type: 'POST',
        	url: "/sys/lss/cnl/sld/removeLssCnlStripLand.do",
        	dataType: "json",
        	success: function(data) {
        		if (data.status == "success") {
                	alert(data.message);
...
function fncUpdateStripLandView(){
	$("#listForm").attr("action","/sys/lss/cnl/sld/updateLssCnlStripLandView.do");
	$("#listForm").submit();
}
