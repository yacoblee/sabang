...
function fnFieldBookDetail(id) {
	...
	$("#listForm").attr("action","/sys/fck/pcs/fbk/selectFieldBookDetail.do");
	$("#listForm").submit();
}
...
//		$.ajax({
//			url:"/sys/fck/pcs/fbk/insertCnrsSpceSld.do",
//			type:"POST",
//	        data: {	        	
//	        	mst_id: mst_id
//	        },
...
function fnUpdateCnrsSpceView(id) {
	...
	$("#listForm").attr("action","/sys/fck/pcs/fbk/updateFieldBookView.do");
	$("#listForm").submit();
}
...
function fnUpdateCnrsSpce(label){
	...
	if(confirm("선택항목을 삭제하시겠습니까?")) {
		$.ajax({
			url:"/sys/fck/pcs/fbk/updateCnrsSpceItem.do",
			type:"POST",
        	data: {
        		label: label,
	        	mst_id: mst_id
...
function fnDeleteCnrsSpce(id){
	if(confirm("공유방을 삭제하시겠습니까?")) {
		if(confirm("공유방삭제시 조사된 정보도 함께 삭제가 됩니다.\n그래도 삭제하시겠습니까?")){
			$.ajax({
				url:"/sys/fck/pcs/fbk/deleteCnrsSpce.do",
				type:"POST",
		        data: {
		        	id: id
		        },
		        ...
		        success:function(data){
		        	if(data.status == "success"){
		        		alert(data.message);
		        		window.location.href = "/sys/fck/pcs/fbk/selectPcsFieldBookList.do"
		        	} else {
		        		alert(data.message);
		        	}
		        },
 ...
 * @Description
 */
function fnSelectFieldBookDetailOne(smid, type){
	$("input[name=smid]").val(smid);
	$("input[name=svyType").val(type);
	var hiddens = $("#listForm").find("input[name*=sch]");
	$.each(hiddens,function(idx,elm){
		$("#listForm").append($('<input/>', {type: 'hidden', name: $(elm).attr("name").replace("sch",""), value:$(elm).val() }));
	});
	
	$("#listForm").attr("action","/sys/fck/pcs/fbk/selectFieldBookDetailOne.do").attr("method", "post");
	$("#listForm").submit();
}
...
function fnSelectFieldBookUpdtOne(smid){
	...
	var hiddens = $("#listForm").find("input[name*=sch]");
	var form = $("<form></form>").attr("action","/sys/fck/pcs/fbk/updateFieldBookViewOne.do").attr("method","post");
...
function fnUpdateFieldBookDetailOne(smid){
	...
	if(!isNaN(bpx)) bpx = svyLonD+(svyLonM/60)+(svyLonS/3600);
	else bpx = "";
	
	var bpy = svyLatD+(svyLatM/60)+(svyLatS/3600);
	if(!isNaN(bpy)) bpy = svyLatD+(svyLatM/60)+(svyLatS/3600);
	else bpy = "";
	...
	$("#listForm").append($('<input/>', {type: 'hidden', name: "svyLon", value:bpx }));
	$("#listForm").append($('<input/>', {type: 'hidden', name: "svyLat", value:bpy }));
	...
	if(confirm("수정하시겠습니까?")) {
		$.ajax({
			url: "/sys/fck/pcs/fbk/updateFieldBookDetailOne.do",
	        ...
	        success:function(data){
	        	if(data.status == "success"){
	        		alert(data.message);
	        		fnList("/sys/fck/pcs/fbk/selectPcsFieldBookList.do");
	        	} else {
	        		alert(data.message);
	        	}
	        },
...
function fnDeleteFieldBookDetailOne(){
	...
	if(confirm("삭제하시겠습니까?")) {
		$.ajax({
			url: "/sys/fck/pcs/fbk/deleteFieldBookDetailOne.do",
			data:$("#listForm").serialize(),
			dataType: "json",
			type:"POST",
			async:true,
			success:function(data){
				if(data.status == "success"){
					alert(data.message);
					window.location.href = "/sys/fck/pcs/fbk/selectFieldBookDetail.do?mstId="+mstId;
				} else {
					alert(data.message);
				}
			},
...
function fncAdministCtprvnNmChange(e){
	...
	var sdNm ; 
	if(e == 'ectrsTable'){
		sdNm = $("#ectrsTable #svySdView").val();
	}else if(e == 'mrngTable'){
		sdNm = $("#mrngTable #svySdView").val();
	}else if(e == 'mtcTable'){
		sdNm = $("#mtcTable #svySdView").val();
	}
        ...
        success:function(list){
        	if(list.result){
        		if(list.result.length > 0){
        			if(e == 'ectrsTable'){
	        			$("#ectrsTable select[name=svySgg]").empty().append("<option value>선택하세요</option>");
	        			$("#ectrsTable select[name=svyEmd]").empty().append("<option value>선택하세요</option>");
	        			$("#ectrsTable select[name=svyRi]").empty().append("<option value>선택하세요</option>");
        			}else if(e == 'mrngTable'){
        				$("#mrngTable select[name=svySgg]").empty().append("<option value>선택하세요</option>");
        				$("#mrngTable select[name=svyEmd]").empty().append("<option value>선택하세요</option>");
        				$("#mrngTable select[name=svyRi]").empty().append("<option value>선택하세요</option>");
        			}else if(e == 'mtcTable'){
        				$("#mtcTable select[name=svySgg]").empty().append("<option value>선택하세요</option>");
        				$("#mtcTable select[name=svyEmd]").empty().append("<option value>선택하세요</option>");
        				$("#mtcTable select[name=svyRi]").empty().append("<option value>선택하세요</option>");
        			}
...
function fnSearch(){
	...
	$("#listForm").attr("action","selectPcsFieldBookList.do");
...
function fnSubSearch(){	
	...
	$("#searchForm").attr("action","/sys/fck/pcs/fbk/selectFieldBookDetail.do");
...
//	$("#listForm").attr("action","/sys/fck/pcs/fbk/selectFieldBookList.do");
//	var params = $("#listForm *[name*=View]");
//	$.each(params,function(idx,elm){
//		var val = $(elm).val();
//		var hidden = $(elm).attr("name").replace("View","");
...
//		url:"/sys/lss/lcp/fbk/selectLcpSldListAddr.do",
		url:"/sys/fck/pcs/fbk/selectMberList.do",
		type:"POST",
        data: {
...
function fnManageCnrsSpceAuthory(id) {
	...
	var form = $("<form></form>").attr("action","/sys/fck/pcs/fbk/selectFieldbookAuthoryView.do").attr("method","post");
	...
	if(confirm("수정하시겠습니까?")) {
		...
		$("#listForm").ajaxForm({
	        ...
	        success: function(data) {
	        	if (data.status == "success") {
                    alert(data.message);
                    //location.href="/sys/fck/pcs/fbk/selectFieldBookList.do";
                    fnList("/sys/fck/pcs/fbk/selectPcsFieldBookList.do");
                } else {
                    alert(data.message);
                }
