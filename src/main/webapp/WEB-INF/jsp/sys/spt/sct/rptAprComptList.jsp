...
<c:set var="pageTitle"><spring:message code="sysSptSct.reportManage.FckAprComptReport"/></c:set>
<div class="wrap-right" id="Cont">
	...
	<div id="contents">
		<fieldset>
			...
			<form id="listForm" action="/sys/spt/sct/selectAprComptReportList.do" method="post">
...
<%-- 					<td class="row"><c:out value="${result.file_orginl_nm}"/></td>					 --%>
					...
					<td class="row">
						<div class="BtnGroup">
				    		<button type="button" class="download-btn-m" onclick="location.href='/sys/spt/sct/selectAprComptReportDownload.do?gid=<c:out value="${result.gid}"/>'"></button>
				    		<sec:authorize access="isAuthenticated()">
					    		<sec:authorize access="hasAnyRole('ROLE_ADMIN_APR','ROLE_SUB_ADMIN','ROLE_ADMIN')">
					    			<button type="button" class="del-btn-m" onclick="javascript:fnDeleteRptFile('<c:out value="${result.gid}"/>','APR')"></button>
					    			<sec:authorize access="!hasAnyRole('ROLE_ADMIN_APR','ROLE_SUB_ADMIN','ROLE_ADMIN')">
...
<%-- 								<sec:authorize access="hasAnyRole('ROLE_USER_APR','ROLE_USER')"> --%>
<%-- 									<c:set var="name" value="${result.file_wrter}" /> --%>
<%-- 									<c:if test="${name eq userNm}"> --%>
<%-- 										<button type="button" class="del-btn" onclick="javascript:fnDeleteRptFile('<c:out value="${result.gid}"/>','APR')"><spring:message code="button.delete" /></button> --%>
<%-- 									</c:if> --%>
<%-- 								</sec:authorize> --%>
							</sec:authorize>
				    	</div>
			...
			<div class="BtnGroup">
				<div class="BtnRightArea">
					<button type="button" class="write-btn" onclick="javascript:fnRptFilePopup('APR');"><spring:message code="title.create"/></button>
				</div>
			</div>
			<div class="Page">
...
<script>
	function linkPage(pageNo){
		...
		$("#listForm").attr("action","<c:url value='/sys/spt/sct/selectAprComptReportList.do'/>");
		$("#listForm").submit();
	}
</script>