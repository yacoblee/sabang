...
<c:set var="pageTitle"><spring:message code="sysSptSct.reportManage.LssBscComptReport"/></c:set>
<div class="wrap-right" id="Cont">
	...
	<div id="contents">
		<fieldset>
			...
			<form id="listForm" action="/sys/spt/sct/selectBscComptReportList.do" method="post">
							...
							<tr>
								...
								<td>
									...
									<select id="searchCondition" name="searchConditionView" title="<spring:message code="title.searchCondition" />">
										...
										<option value="1"<c:if test="${searchVO.searchCondition == '1'}">selected="selected"</c:if>><spring:message code="mngRptBsc.reportManage.file_orginl_nm"/></option>
										<option value="2"<c:if test="${searchVO.searchCondition == '2'}">selected="selected"</c:if>><spring:message	code="mngRptBsc.reportManage.file_wrter"/></option>
								</select>
		...
		<div class="BoardList">
			...
			<table summary="<spring:message code="common.summary.inqire" arguments="${pageTitle}" />">
				<colgroup>
					<col style="width: 36.3px;">
					<col style="width: 72.6x;">
					<col style="width: 72.6px;">
					<col style="width: 217.8px;">
				...
				<thead>
					<tr>
						...
						<th><spring:message code="sysSptSct.reportFile.corpNm" /></th>
...
<%-- 						<th><spring:message code="sysSptSct.reportFile.file_orginl_nm" /></th> --%>
				...
				<tr>
					<td colspan="9" class="center"><spring:message code="info.nodata.msg" /></td>
				</tr>
				</c:if>
...
<%-- 					<td class="row"><c:out value="${result.file_orginl_nm}"/></td>					 --%>
					...
					<td class="row">
						<div class="BtnGroup">
				    		<button type="button" class="download-btn-m" onclick="location.href='/sys/spt/sct/selectBscComptReportDownload.do?gid=<c:out value="${result.gid}"/>'"></button>
				    		<sec:authorize access="isAuthenticated()">
					    		<sec:authorize access="hasAnyRole('ROLE_ADMIN_BSC','ROLE_SUB_ADMIN','ROLE_ADMIN')">
										<button type="button" class="del-btn-m" onclick="javascript:fnDeleteRptFile('<c:out value="${result.gid}"/>','BSC')"></button>
										<sec:authorize access="!hasAnyRole('ROLE_ADMIN_BSC','ROLE_SUB_ADMIN','ROLE_ADMIN')">
...
<%-- 								<sec:authorize access="hasAnyRole('ROLE_USER_BSC','ROLE_USER')"> --%>
<%-- 									<c:set var="name" value="${result.file_wrter}" /> --%>
<%-- 									<c:if test="${name eq userNm}"> --%>
<%-- 										<button type="button" class="del-btn" onclick="javascript:fnDeleteRptFile('<c:out value="${result.gid}"/>','BSC')"><spring:message code="button.delete" /></button> --%>
<%-- 									</c:if> --%>
<%-- 								</sec:authorize> --%>
							</sec:authorize>
				    	</div>
			...
			<div class="BtnGroup">
				<div class="BtnRightArea">
					<button type="button" class="write-btn" onclick="javascript:fnRptFilePopup('BSC');"><spring:message code="title.create"/></button>
				</div>
			</div>
			<div class="Page">
...
<script>
	function linkPage(pageNo){
		...
		$("#listForm").attr("action","<c:url value='/sys/spt/sct/selectBscComptReportList.do'/>");
		$("#listForm").submit();
	}
</script>