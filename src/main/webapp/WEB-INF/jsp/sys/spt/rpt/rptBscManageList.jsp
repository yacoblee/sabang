	...
	<div id="contents">
		<fieldset>
			...
			<form id="listForm" action="/sys/spt/rpt/selectRptBscManageList.do" method="post">
				...
				<input name="ncssty" type="hidden" value="<c:out value='${searchVO.ncssty}'/>"/>
				<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>">			
				<input name="rptType" type="hidden" value="<c:out value='${pageTitle}'/>"/>
				<input name="mstNmList" type="hidden"/>
				<input name="svyIdList" type="hidden"/>
				<input name="pageVal" type="hidden" value="150">
				<div class="search">
					<table  class="tableWid_L" summary="<spring:message code="common.search.summary.list" arguments="${pageTitle}" />">
						...
						<tbody>
							<tr>
								<th><spring:message code="sysSptRpt.reportManage.svyType"/><span class="pilsu">*</span></th><!-- 조사유형 -->
								<td>
								...
								<td colspan="3">
									<label for="region1View" class="Hidden"><spring:message code="sysSptRpt.reportManage.svyRegion1"/></label>
									<select id="region1View" name="region1View" title="<spring:message code="sysSptRpt.reportManage.svyRegion1"/>" onchange="fncRegionChange(event,'list'); return false;">
									...
									<label for="region2View" class="Hidden"><spring:message code="sysSptRpt.reportManage.svyRegion1"/></label>
									<select id="region2View" name="region2View" title="<spring:message code="sysSptRpt.reportManage.svyRegion1"/>" >
							...
							<tr>
								...
								<th class="ncsstyPassAt"><spring:message code="sysLssBsc.svyComptVO.ncsstyPassAt"/></th><!-- 실태조사 패스 제외여부 -->
								<td>
									<label for="sysLssBsc.svyComptVO.ncsstyPassAt" class="Hidden"><spring:message code="sysLssBsc.svyComptVO.ncsstyPassAt"/></label>
									<input type="checkbox" name="ncsstyPassAt" style="min-width:60px; height:20px;" <c:if test="${searchVO.ncsstyPassAt eq '패스'}">checked</c:if>/>
								</td>
							</tr>
							<tr>
		...
		<div class="BoardList">
			<span class="ListCount">Total : <strong><c:out value="${paginationInfo.totalRecordCount}"/></strong> Page : <strong><c:out value="${paginationInfo.currentPageNo}"/>/<c:out value="${paginationInfo.totalPageCount}"/></strong>
				...
				<button type="button" class="download-btn fr" onclick="javascript:fncAllCheckReport('BSC'); return false;">전체보고서</button>
				<button type="button" class="download-btn fr" onclick="javascript:fncMultiCheckReport('BSC'); return false;">보고서</button>
...
<script>
	...
	$(document).ready(function() {
		if($('input[name=ncssty]').val().length > 0){
			$("input[name=ncsstyPassAt]").prop("checked",false).css({'display':'none'});
			$('.ncsstyPassAt').css({'visibility':'hidden'});
		}
	});
	$(document).on("change","select[name=ncsstyView]",function(){
		if($(this).val().length > 0 ){
			$("input[name=ncsstyPassAt]").prop("checked",false).css({'display':'none'});
			$('.ncsstyPassAt').css({'visibility':'hidden'});
		}else{
			$("input[name=ncsstyPassAt]").css({'display':'block'});
			$('.ncsstyPassAt').css({'visibility':'visible'});
		};
	});
