...
<c:set var="pageTitle"><spring:message code="sysSptRpt.reportManage.LssWkaReport"/></c:set>
<div class="wrap-right" id="Cont">
	...
	<div id="contents">
		<fieldset>
			...
			<form id="listForm" action="/sys/spt/rpt/selectRptWkaManageList.do" method="post">
				...
				<div class="search">
					<table  class="tableWid_L" summary="<spring:message code="common.search.summary.list" arguments="${pageTitle}" />">
						...
						<tbody>
							...
							<tr>
								<th><spring:message code="sysLssWka.stripLandVO.addr"/></th><!-- 주소 -->
								<td colspan="4">
							...
							<tr>
								...
								<th><spring:message code="sysLssWka.svyComptVO.mstNm"/></th><!-- 공유방명 -->
								<td colspan="2">
									<label for="sysLssWka.svyComptVO.mstNm" class="Hidden"><spring:message code="sysLssWka.svyComptVO.mstNm"/></label>
		...
		<div class="BoardList">
			<span class="ListCount">Total : <strong><c:out value="${paginationInfo.totalRecordCount}"/></strong> Page : <strong><c:out value="${paginationInfo.currentPageNo}"/>/<c:out value="${paginationInfo.totalPageCount}"/></strong>
				...
				<button type="button" class="download-btn fr" onclick="javascript:fncAllCheckReport('WKA'); return false;">전체보고서</button>
				<button type="button" class="download-btn fr" onclick="javascript:fncMultiCheckReport('WKA'); return false;">보고서</button>
...
<script>
	function linkPage(pageNo){
		...
		$("#listForm").attr("action","<c:url value='/sys/spt/rpt/selectRptWkaManageList.do'/>");
		$("#listForm").submit();
	};
</script>