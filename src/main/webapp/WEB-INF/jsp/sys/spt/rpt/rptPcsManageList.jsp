...
<c:set var="pageTitle"><spring:message code="sysSptRpt.reportManage.FckPcsReport"/></c:set>
<div class="wrap-right" id="Cont">
	...
	<div id="contents">
		<fieldset>
			...
			<form id="listForm" action="/sys/spt/rpt/selectRptPcsManageList.do" method="post">
				...
				<input name="rptType" type="hidden" value="${pageTitle}" />
							...
							<tr>
								...
								<td>
									...
									<select id="typeView" name="typeView" title="<spring:message code="sysSptRpt.reportManage.svyType"/>">
										...
										<option value="콘크리트 사방댐 정밀점검"<c:if test="${searchVO.type eq '콘크리트 사방댐 정밀점검'}">selected="selected"</c:if>>콘크리트 사방댐 정밀점검</option>
										<option value="석재 사방댐 정밀점검"<c:if test="${searchVO.type eq '석재 사방댐 정밀점검'}">selected="selected"</c:if>>석재 사방댐 정밀점검</option>
										<option value="강재 사방댐 정밀점검"<c:if test="${searchVO.type eq '강재 사방댐 정밀점검'}">selected="selected"</c:if>>강재 사방댐 정밀점검</option>
										<option value="콘크리트 사방댐 정밀점검"<c:if test="${searchVO.type eq '사방댐 정밀점검'}">selected="selected"</c:if>>사방댐 정밀점검</option>
										<option value="계류보전 정밀점검"<c:if test="${searchVO.type eq '계류보전 정밀점검'}">selected="selected"</c:if>>계류보전 정밀점검</option>
										<option value="산지사방 정밀점검"<c:if test="${searchVO.type eq '산지사방 정밀점검'}">selected="selected"</c:if>>산지사방 정밀점검</option>
										<option value="해안방재림 정밀점검"<c:if test="${searchVO.type eq '해안방재림 정밀점검'}">selected="selected"</c:if>>해안방재림 정밀점검</option>
										<option value="해안침식방지 정밀점검"<c:if test="${searchVO.type eq '해안침식방지 정밀점검'}">selected="selected"</c:if>>해안침식방지 정밀점검</option>
									</select>
								</td>
		...
		<div class="BoardList">
			<span class="ListCount">Total : <strong><c:out value="${paginationInfo.totalRecordCount}"/></strong> Page : <strong><c:out value="${paginationInfo.currentPageNo}"/>/<c:out value="${paginationInfo.totalPageCount}"/></strong>
				...
				<button type="button" class="download-btn fr" onclick="javascript:fncAllCheckReport('PCS'); return false;">전체보고서</button>
				<button type="button" class="download-btn fr" onclick="javascript:fncMultiCheckReport('PCS'); return false;">보고서</button>
...
<script>
	function linkPage(pageNo){
		...
		$("#listForm").attr("action","<c:url value='/sys/spt/rpt/selectRptPcsManageList.do'/>");
		$("#listForm").submit();
	}
</script>