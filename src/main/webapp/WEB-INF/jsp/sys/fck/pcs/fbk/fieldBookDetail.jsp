...
<c:set var="pageTitle"><spring:message code="sysFckPcs.fieldBookDetail.title"/></c:set>
...
<div class="wrap-right" id="Cont">
	...
	<div id="contents">
		...
		<legend class="Hidden">검색테이블</legend>
			<form id="searchForm" action="/sys/fck/pcs/fbk/selectFieldBookDetail.do" method="post">
				...
				<input name="ri" type="hidden" value="<c:out value='${searchItemVO.ri}'/>"/>
					<div class="search">
						<table  class="tableWid_M" summary="<spring:message code="common.search.summary.list" arguments="${pageTitle}" />">
							...
							<tbody>
							<tr>
								<th><spring:message code="sysFckPcs.fieldBookItemVO.svyid"/></th><!-- 조사 ID/라벨 -->
									<td>
										<label for="sysFckPcs.fieldBookItemVO.svyid" class="Hidden"><spring:message code="sysFckPcs.fieldBookItemVO.svyid"/></label>
								...
								<th><spring:message code="sysLssLcp.fieldBookItemVO.addr"/></th><!-- 주소 -->
									<td colspan="5">
										<label for="sdView" class="Hidden"><spring:message code="sysFckPcs.fieldBookItemVO.sd"/></label>
										<select id="sdView" name="sdView" title="<spring:message code="sysFckPcs.fieldBookItemVO.sd"/>" onchange="fncAdministCtprvnChange(event); return false;">
										...
										<label for="sggView" class="Hidden"><spring:message code="sysFckPcs.fieldBookItemVO.sgg"/></label>
										<select id="sggView" name="sggView" title="<spring:message code="sysFckPcs.fieldBookItemVO.sgg"/>" onchange="fncAdministSignguChange(event); return false;">
										...
										<label for="emdView" class="Hidden"><spring:message code="sysFckPcs.fieldBookItemVO.emd"/></label>
										<select id="emdView" name="emdView" title="<spring:message code="sysFckPcs.fieldBookItemVO.emd"/>" onchange="fncAdministEmdChange(event); return false;">
										...
										<label for="riView" class="Hidden"><spring:message code="sysFckPcs.fieldBookItemVO.ri"/></label>
										<select id="riView" name="riView" title="<spring:message code="sysFckPcs.fieldBookItemVO.ri"/>">
			...
			<div class="BoardDetail">
				...
				<caption class="Hidden">${pageTitle} <spring:message code="title.detail" /></caption>
					<tr>
						<th><spring:message code="sysFckPcs.fieldBookVO.id" /></th>
						<td><c:out value="${result.id}"/></td>
						<th><spring:message code="sysFckPcs.fieldBookVO.create_user" /></th>
						<td><c:out value="${result.mst_create_user}"/></td>
						<th><spring:message code="sysFckPcs.fieldBookVO.registered" /></th>
						<td><c:out value="${result.mst_registered}"/></td>
					</tr>
					<tr>
						<th><spring:message code="sysFckPcs.fieldBookVO.corpname" /></th>
						<td><c:out value="${result.mst_corpname}"/></td>
						<th><spring:message code="sysFckPcs.fieldBookVO.partname" /></th>
						<td><c:out value="${result.mst_partname}"/></td>
						<th><spring:message code="sysFckPcs.fieldBookVO.totcnt" /></th>
						<td><c:out value="${result.totcnt}"/></td>
					</tr>
				</table>
			</div>
			<form id="listForm" action="/sys/fck/pcs/fbk/selectFieldBookDetail.do" method="post">
				<input name="smid" type="hidden" value="">
				...
				<input name="pageSubIndex" type="hidden" value="<c:out value='${searchItemVO.pageSubIndex}'/>">
				<input name="svyType" type="hidden" value=""/>
			...
			<span class="ListCount">Total : <strong><c:out value="${paginationInfo.totalRecordCount}"/></strong> Page : <strong><c:out value="${paginationInfo.currentPageNo}"/>/<c:out value="${paginationInfo.totalPageCount}"/></strong>
				...
				<button style="float:right;" type="button" class="add-btn" onclick="javascript:fnManageCnrsSpceAuthory('<c:out value="${result.id}"/>');"><spring:message code="sysFckPcs.stripLand.authorManage" /></button>
				<button style="float:right;" type="button" class="save-btn" onclick="javascript:fnInsertCnrsSpceSldPopup('<c:out value="${result.id}"/>');"><spring:message code="sysFckPcs.stripLand.upload" /></button>
				</sec:authorize>
				<sec:authorize access="!hasAnyRole('ROLE_ADMIN_APR','ROLE_SUB_ADMIN','ROLE_ADMIN')">
			...
			<table summary="<spring:message code="common.summary.inqire" arguments="${subTitle}" />">
				...
				<thead>
					<tr>
						<th><spring:message code="sysFckPcs.fieldBookItemVO.cnt" /></th>
				...
				<tbody class="ov">
					...
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr>
							...
							<td>
								<div class="BtnGroup">
									<button type="button" class="search-btn-m" onclick="fnSelectFieldBookDetailOne('<c:out value="${result.smid}"/>','<c:out value="${result.type }"/>');"></button>
								</div>
							</td>					
						</tr>
					</c:forEach>
			...
			<div class="BtnGroup">
				<div class="BtnRightArea">
					<sec:authorize access="hasAnyRole('ROLE_ADMIN_APR','ROLE_SUB_ADMIN','ROLE_ADMIN')">
					<button type="button" class="del-btn" onclick="javascript:fnDeleteCnrsSpce('<c:out value="${mstId}"/>');"><spring:message code="sysFckPcs.deleteFieldBook.title" /></button>
					</sec:authorize>
