...
<div class="sub-upload">
	<div class="BoardDetail">
		<form:form id="insertForm" modelAttribute="fieldBookVO" commandName="fieldBookVO" method="post" action="/sys/fck/pcs/fbk/insertCnrsSpce.do">
			<input type="hidden" name="mst_corpname" value="한국치산기술협회"/>
			<input type="hidden" name="svytype" value="PCS" />
			<table class="fieldBookTable ov">
				<colgroup>
		      	...
		      	<!-- 사업지구명 -->
          	  	<tr>
			      	<c:set var="title"><spring:message code="sysFckPcs.fieldBookVO.partname"/></c:set>
				  	<th>${title}</th>
				  	<td>
		      	...
		      	<!-- 부서명 -->
				<tr>
					<c:set var="title"><spring:message code="sysFckPcs.fieldBookVO.deptname"/></c:set>
					<th>${title}</th>
					<td>
		      	...
		      	<!-- 생성자명 -->
          	  	<tr>
			      	<c:set var="title"><spring:message code="sysFckPcs.fieldBookVO.create_user"/></c:set>
				  	<th>${title}</th>
				  	<td>
          	   	...
          	   	<!-- 비밀번호 -->
          	  	<tr>
			      	<c:set var="title"><spring:message code="sysFckPcs.fieldBookVO.password"/></c:set>
				  	<th>${title}</th>
				  	<td>
				...
				<!-- 권한자 -->
				<tr class="searchList">
					<c:set var="title"><spring:message code="sysFckPcs.fieldBookVO.author_setup"/></c:set>
					<th>${title}</th>
					<td>
						<select id="searchCondition" name="searchCondition" title="<spring:message code="sysFckPcs.fieldBookVO.searchConditioTitle" />" style="width:30%;">
							...
							<option value="1" <c:if test="${userSearchVO.searchCondition == '1'}">selected="selected"</c:if>><spring:message code="sysFckPcs.fieldBookVO.deptname" /></option>
							<option value="2" <c:if test="${userSearchVO.searchCondition == '2'}">selected="selected"</c:if>><spring:message code="sysFckPcs.fieldBookVO.name" /></option>
						</select>
				...
				<tr>
					<c:set var="title"><spring:message code="sysFckPcs.fieldBookVO.authory"/></c:set>
					<th>${title}</th>
					<td>
