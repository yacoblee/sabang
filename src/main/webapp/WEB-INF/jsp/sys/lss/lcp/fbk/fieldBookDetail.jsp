...
<div class="wrap-right" id="Cont">
	...
	<fieldset>
		<legend class="Hidden">검색테이블</legend>
			<form id="searchForm" action="/sys/lss/lcp/fbk/selectFieldBookDetail.do" method="post">
				...
				<input name="svyRi" type="hidden" value="<c:out value='${searchVO.svyRi}'/>"/>
					...
					<div class="search">
						<table  class="tableWid_M" summary="<spring:message code="common.search.summary.list" arguments="${pageTitle}" />">
							...
							<tbody>
								<tr>
									<th><spring:message code="sysLssLcp.stripLandVO.label"/></th><!-- 조사 ID/라벨 -->
										<td>
											<label for="sysLssLcp.stripLandVO.label" class="Hidden"><spring:message code="sysLssLcp.stripLandVO.label"/></label>
									...
									<th><spring:message code="sysLssLcp.fieldBookItemVO.addr"/></th><!-- 주소 -->
										<td colspan="5">
											<label for="svySdView" class="Hidden"><spring:message code="sysLssLcp.svyComptVO.sd"/></label>
											<select id="svySdView" name="svySdView" title="<spring:message code="sysLssLcp.fieldBookItemVO.svySd"/>" onchange="fncAdministNmCtprvnChange(event); return false;">
											...
											<label for="svySggView" class="Hidden"><spring:message code="sysLssLcp.fieldBookItemVO.svySgg"/></label>
											<select id="svySggView" name="svySggView" title="<spring:message code="sysLssLcp.fieldBookItemVO.svySgg"/>" onchange="fncAdministNmSignguChange(event); return false;">
											...
											<label for="svyEmdView" class="Hidden"><spring:message code="sysLssLcp.fieldBookItemVO.svyEmd"/></label>
											<select id="svyEmdView" name="svyEmdView" title="<spring:message code="sysLssLcp.fieldBookItemVO.svyEmd"/>" onchange="fncAdministNmEmdChange(event); return false;">
											...
											<label for="svyRiView" class="Hidden"><spring:message code="sysLssLcp.fieldBookItemVO.svyRi"/></label>
											<select id="svyRiView" name="svyRiView" title="<spring:message code="sysLssLcp.fieldBookItemVO.svyRi"/>">
			...
			<fieldset>
			<form id="listForm" action="/sys/lss/lcp/fbk/selectFieldBookDetail.do" method="post">
			...
			<span class="ListCount">Total : <strong><c:out value="${paginationInfo.totalRecordCount}"/></strong> Page : <strong><c:out value="${paginationInfo.currentPageNo}"/>/<c:out value="${paginationInfo.totalPageCount}"/></strong>
				<sec:authorize access="hasAnyRole('ROLE_ADMIN_LCP','ROLE_SUB_ADMIN','ROLE_ADMIN')">
					<button style="float:right;" type="button" class="add-btn" onclick="javascript:fnManageCnrsSpceAuthory('<c:out value="${result.id}"/>');"><spring:message code="sysLssLcp.stripLand.authorManage" /></button>
					<button style="float:right;" type="button" class="save-btn" onclick="javascript:fnInsertCnrsSpceSldPopup('<c:out value="${result.id}"/>');"><spring:message code="sysLssLcp.stripLand.upload" /></button>
				</sec:authorize>
				<sec:authorize access="!hasAnyRole('ROLE_ADMIN_LCP','ROLE_SUB_ADMIN','ROLE_ADMIN')">
			...
			<table summary="<spring:message code="common.summary.inqire" arguments="${subTitle}" />">
				...
				<thead>
					<tr>
						<th><spring:message code="sysLssLcp.fieldBookItemVO.cnt" /></th>
