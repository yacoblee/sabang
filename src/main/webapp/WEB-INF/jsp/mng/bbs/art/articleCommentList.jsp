...
<!-- 댓글 내용  -->
	<c:set var="replyTitle"><%-- <spring:message code="comCopCmt.articleCommentVO.title"/> --%>댓글이 없습니다.</c:set>
	<h3>${replyTitle} <c:out value="${cmtCnt}"/></h3>
	<div class="reply">
		<ul>
			<c:if test="${fn:length(cmtList) == 0}">
				<li>
			  		<p class="txt"><%-- <spring:message code="common.nocomment.msg" /> --%> 댓글이없습니다.</p>
		  		</li>
			</c:if>
