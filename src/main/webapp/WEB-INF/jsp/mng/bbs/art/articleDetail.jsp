...
<div class="wrap-right" id="Cont">
	...
	<div id="contents">	
		<form id="listForm" action="${pageContext.request.contextPath}/mng/bbs/art/selectArticleDetail.do" method="post">
			<input name="subPageIndex" type="hidden" value="<c:out value='${articleCommentVO.subPageIndex}'/>">
			<input type="hidden" name="bbsId" value="<c:out value="${result.bbsId}"/>"/>
			<input type="hidden" name="nttId" value="<c:out value="${result.nttId}"/>"/>
			<input name="searchCondition" type="hidden" value="<c:out value='${result.searchCondition}'/>"/>
			<input name="searchKeyword" type="hidden" value="<c:out value='${result.searchKeyword}'/>"/>
			...
			<table summary="<spring:message code="common.summary.inqire" arguments="${pageTitle}" />">
				...
				<tbody>
					<!-- 글 제목  -->
					<c:set var="title"><spring:message code="mngBbs.articleVO.detail.nttSj" /></c:set>
					<tr>
						<th>${title}</th>
					...
					<tr>
						...
						<th><spring:message code="mngBbs.articleVO.detail.rdcnt"/></th>
						<td class="left"><c:out value="${result.rdcnt}"/></td>
					</tr>
					<!-- 글 내용 -->
					<c:set var="title"><spring:message code="mngBbs.articleVO.detail.nttCn" /></c:set>
					<tr>
						<th class="vtop">${title}</th>
						<td colspan="5" class="cnt"><textarea style="resize:none; min-height: 300px; width: 100%; border: 1px solid #ddd;" cols="300" rows="20" readonly="readonly" disabled="disabled"><c:out value="${result.nttCn}"/></textarea></td>
					</tr>
					<!-- 첨부파일  -->
					<c:if test="${not empty result.atchFileId}">
					<tr>
						<th><spring:message code="mngBbs.articleVO.detail.atchFile" /></th>
						<td colspan="5">
							<c:import url="/mng/bbs/art/selectFileInfs.do" charEncoding="utf-8">
		...
		<!-- 댓글 작성 폼 -->
		<form:form commandName="articleCommentVO" action="${pageContext.request.contextPath}/mng/bbs/art/insertArticleComment.do" method="post">
			<input type="hidden" name="bbsId" value="<c:out value="${result.bbsId}"/>"/>
			<input type="hidden" name="nttId" value="<c:out value="${result.nttId}"/>"/>
