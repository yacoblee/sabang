...
<div class="wrap-right" id="Cont">
	...
	<div id="contents">
		<fieldset>
			...
			<form id="listForm" action="/mng/umt/mberManage.do" method="post">
							...
							<tr>
								...
								<td>
									...
									<select id="searchConditionView" name="searchConditionView" title="<spring:message code="mngUmt.userManageSsearch.searchConditioTitle" />">
										...
										<option value="3" <c:if test="${userSearchVO.searchCondition == '3'}">selected="selected"</c:if>><spring:message code="mngUmt.userManageList.phone" /></option>
									</select>
		...
		<div class="BoardList">
			...
			<tr>
				...
				<th><spring:message code="mngUmt.userManageList.sbscrbSttus" /></th><!-- 가입상태 -->
				<th><spring:message code="mngUmt.userManageList.edit" /></th><!-- 관리 -->
			</tr>
			</thead>
			<tbody class="ov">
			...
			<tr>
			    ...
			    <td>
			        <c:forEach var="memberSttus_result" items="${mberSttus_result}" varStatus="status">
			            <c:if test="${result.sttus == memberSttus_result.code}"><c:out value="${memberSttus_result.codeNm}"/></c:if>
			        </c:forEach>
			    </td>
			    <td>
				    <div class="BtnGroup">
						<button type="button" class="modify-btn" onclick="javascript:fnSelectMember('<c:out value="${result.uniqId}"/>'); return false;"><spring:message code="title.update" /></button>
						<button type="button" class="del-btn" onclick="javascript:fnDeleteMember('<c:out value="${result.userId}"/>','<c:out value="${result.uniqId}"/>'); return false;"><spring:message code="title.delete" /></button>
				    </div>
			    </td>
			</tr>
			</c:forEach>
	...
	<script>
		...
		function linkPage(pageNo){
			...
			$("#listForm").attr("action","<c:url value='/mng/umt/mberManage.do'/>");
			$("#listForm").submit();
		}
	</script>
</div>