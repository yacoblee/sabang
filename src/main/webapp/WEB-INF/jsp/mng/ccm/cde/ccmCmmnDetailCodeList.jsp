...
<div class="wrap-right" id="Cont">
	...
	<div id="contents">
		<fieldset>
			...
			<form id="listForm" action="<c:url value='/mng/ccm/cde/selectCcmCmmnDetailCodeList.do'/>" method="post">
							...
							<tr>
								...
								<td>
									...
									<select id="searchCondition" name="searchConditionView" title="<spring:message code="title.searchCondition" />">
										...
										<option value="1" <c:if test="${searchVO.searchCondition == '1'}">selected="selected"</c:if>><spring:message code="mngCcmCde.cmmnDetailCodeVO.codeId"/></option>
										<option value="2" <c:if test="${searchVO.searchCondition == '2'}">selected="selected"</c:if>><spring:message code="mngCcmCde.cmmnDetailCodeVO.code"/></option>
										<option value="3" <c:if test="${searchVO.searchCondition == '3'}">selected="selected"</c:if>><spring:message code="mngCcmCde.cmmnDetailCodeVO.codeNm"/></option>
									</select>
		...
		<div class="BoardList">
			...
			<caption class="Hidden">${pageTitle} <spring:message code="title.list" /></caption>
				...
				<thead>
					<tr>
						...
						<th><spring:message code="mngCcmCde.cmmnDetailCodeVO.codeId" /></th><!-- 코드ID -->
						<th><spring:message code="mngCcmCde.cmmnDetailCodeVO.code" /></th><!-- 코드 -->
						<th><spring:message code="mngCcmCde.cmmnDetailCodeVO.codeNm"/></th><!-- 코드명 -->
						<th><spring:message code="mngCcmCde.cmmnDetailCodeVO.useAt" /></th><!-- 사용여부 -->
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						...
						<td><a href="<c:url value='/mng/ccm/cde/selectCcmCmmnDetailCodeDetail.do'/>?codeId=${resultInfo.codeId}&amp;code=${resultInfo.code}" onClick="fnDetailCodedetail('<c:out value="${resultInfo.codeId}"/>','<c:out value="${resultInfo.code}"/>');return false;"><c:out value='${fn:substring(resultInfo.codeNm, 0, 40)}'/></a></td>
			...
			<div class="BtnGroup">
				<div class="BtnRightArea">
					<button type="button" class="write-btn" onclick="javascript:fncAddCmmnDetailCodeInsert();"><spring:message code="button.create" /></button>
				</div>
			</div>
			<div class="Page">
