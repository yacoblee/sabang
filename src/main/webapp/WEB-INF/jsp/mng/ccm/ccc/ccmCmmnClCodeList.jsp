...
<div class="wrap-right" id="Cont">
	...
	<div id="contents">
		<fieldset>
			...
			<form id="listForm" action="/mng/ccm/ccc/selectCcmCmmnClCodeList.do" method="post">
							...
							<tr>
								...
								<td>
									...
									<select id="searchCondition" name="searchConditionView" title="<spring:message code="title.searchCondition" />">
										...
										<option value="1" <c:if test="${searchVO.searchCondition == '1'}">selected="selected"</c:if>><spring:message code="mngCcmCcc.cmmnClCodeVO.clCode"/></option>
										<option value="2" <c:if test="${searchVO.searchCondition == '2'}">selected="selected"</c:if>><spring:message code="mngCcmCcc.cmmnClCodeVO.clCodeNm"/></option>
									</select>
		...
		<div class="BoardList">
			...
			<caption class="Hidden">${pageTitle} <spring:message code="title.list" /></caption>
				...
				<thead>
					<tr>
						...
						<th><spring:message code="mngCcmCcc.cmmnClCodeVO.clCode" /></th><!-- 분류코드 -->
						<th class="board_th_link"><spring:message code="mngCcmCcc.cmmnClCodeVO.clCodeNm" /></th><!-- 분류코드명 -->
						<th><spring:message code="mngCcmCcc.cmmnClCodeVO.useAt" /></th><!-- 사용여부 -->
					</tr>
				</thead>
				<tbody>
				...
				<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
					<tr>
						...
						<td><a href="<c:url value='/mng/ccm/ccc/selectCcmCmmnClCodeDetail.do'/>?clCode=${resultInfo.clCode}" onClick="fnClCodedetail('<c:out value="${resultInfo.clCode}"/>');return false;"><c:out value='${fn:substring(resultInfo.clCodeNm, 0, 40)}'/></a></td>
			...
			<div class="BtnGroup">
				<div class="BtnRightArea">
					<button type="button" class="write-btn" onclick="javascript:fncAddCmmnClCodeInsert();"><spring:message code="button.create" /></button>
				</div>
			</div>
			<div class="Page">
