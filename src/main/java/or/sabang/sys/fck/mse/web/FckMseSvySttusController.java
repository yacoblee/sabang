...
@Controller
public class FckMseSvySttusController {
	...
	@Resource(name = "fckMseSvySttusService")
	private FckMseSvySttusService fckMseSvySttusService;
	
	private static final Logger LOGGER = LoggerFactory.getLogger(FckMseSvySttusController.class);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/sys/fck/mse/sts/selectMseSvySttus.do")	
    public String selectBscSvySttus (ModelMap model) throws Exception {
		
		List<EgovMap> allSvyList = fckMseSvySttusService.selectMseSvySttusTotCnt();
		List<EgovMap> regionList = fckMseSvySttusService.selectMseSvySttusRegion();
		List<EgovMap> monthList = fckMseSvySttusService.selectMseSvySttusMonth();
		...
		return "sys/fck/mse/sts/surveySttusList";		
	}
}
